  ...
  think(input) {
    ...
    for (let i = 0; i < this.body.guns.length; i++) {
      if (this.body.guns[i].canShoot && !this.body.aiSettings.skynet) {
        ...
        tracking = v.speed;
        if (true) range = 640 * this.body.FOV;else range = Math.min(range, (v.speed || 1) * (v.range || 90));
        break;
      }
    }
...
var gameloop = (() => {
  ...
  let collide = (() => {
    ...
    return collision => {
      ...
      } else if (instance.team === other.team && (instance.settings.hitsOwnType === "pushOnlyTeam" || other.settings.hitsOwnType === "pushOnlyTeam")) {
...
var maintainloop = (() => {
  ...
  let createDom = (loc, mode, type) => {
    ...
    o.ondead = () => {
      ...
      for (let body of entities) if (body.team === pwned && body.type === 'tank') body.sendMessage('Press G to control the Dominator!', [3, 10, 11, 12, 15, 5, 19, 52, 2][-pwned]);
