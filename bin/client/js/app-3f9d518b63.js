	...
	var drawEntity = function () {
		...
		function drawPoly(context, centerX, centerY, radius, sides) {
			...
			if (!sides) {
				...
				for (var _i4 = 0; _i4 < 6; _i4++) {
					var _theta = _i4 / 6 * 2 * Math.PI,
					    x = centerX + radius * 1.1 * Math.cos(180 / 6 + _theta + angle + 0.385),
					    y = centerY + radius * 1.1 * Math.sin(180 / 6 + _theta + angle + 0.385);
					context.lineTo(x, y);
				}
			} else if (sides > 0) {
				// Polygon
				for (var _i5 = 0; _i5 < sides; _i5++) {
					var _theta2 = _i5 / sides * 2 * Math.PI;
					var _x17 = centerX + radius * Math.cos(_theta2 + angle);
					var _y = centerY + radius * Math.sin(_theta2 + angle);
					context.lineTo(_x17, _y);
				}
			}
			context.closePath();
			context.stroke();
		...
		return function (x, y, instance, ratio) {
			...
			if (source.guns.length === m.guns.length) {
				...
				for (var _i6 = 0; _i6 < m.guns.length; _i6++) {
					var g = m.guns[_i6],
					    position = positions[_i6] / (g.aspect === 1 ? 2 : 1),
			...
			if (source.turrets.length === m.turrets.length) {
				for (var _i7 = 0; _i7 < m.turrets.length; _i7++) {
					var _t = m.turrets[_i7];
					if (_t.layer === 1) {
						var _ang = _t.direction + _t.angle + rot,
						    _len4 = _t.offset * drawSize;
						drawEntity(xx + _len4 * Math.cos(_ang), yy + _len4 * Math.sin(_ang), _t, ratio, drawSize / ratio / _t.size * _t.sizeFactor, source.turrets[_i7].facing + turretsObeyRot * rot, turretsObeyRot, context, source.turrets[_i7], render);
					}
				}
			} else {
				throw new Error("Mismatch turret number with mockup.");
	...
	var gameDraw = function () {
		...
		return function (ratio) {
...

			{
				...
				var len = 0;
				var height = 18;
				var _x26 = global.screenWidth / 2;
				var _y2 = spacing;
				// Draw each message
				for (var _i8 = messages.length - 1; _i8 >= 0; _i8--) {
					var msg = messages[_i8],
					    txt = msg.text,
					...
					drawBar(_x26 - msg.len / 2, _x26 + msg.len / 2, _y2 + height / 2, height, color.black);
					// Draw the text
					ctx.globalAlpha = Math.min(1, msg.alpha);
					msg.textobj.draw(_text, _x26, _y2 + height / 2, height - 4, color.guiwhite, 'center', true);
					// Iterate and move
					_y2 += vspacing + height;
					if (msg.status > 1) {
						_y2 -= (vspacing + height) * (1 - Math.sqrt(msg.alpha));
					}
					if (msg.status > 1) {
						msg.status -= 0.05;
						msg.alpha += 0.05;
					} else if (_i8 === 0 && (messages.length > 5 || Date.now() - msg.time > 10000)) {
						msg.status -= 0.05;
						msg.alpha -= 0.05;
						// Remove
						if (msg.alpha <= 0) {
...

			{
				...
				var _y3 = global.screenHeight - spacing - _height;
				var ticker = 11;
				...
				_gui.skills.forEach(function drawASkillBar(skill) {
					...
					if (cap) {
						...
						drawBar(_x27 + _height / 2, _x27 - _height / 2 + _len5 * ska(cap), _y3 + _height / 2, _height - 3 + config.graphical.barChunk, color.black);
						drawBar(_x27 + _height / 2, _x27 + _height / 2 + (_len5 - gap) * ska(cap), _y3 + _height / 2, _height - 3, color.grey);
						drawBar(_x27 + _height / 2, _x27 + _height / 2 + (_len5 - gap) * ska(level), _y3 + _height / 2, _height - 3.5, col);
						// Blocked-off area
						if (blocking) {
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x27 + (_len5 - gap) * ska(j), _y3 + 1.5, _x27 + (_len5 - gap) * ska(j), _y3 - 3 + _height);
							}
						}
						// Vertical dividers
						...
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x27 + (_len5 - gap) * ska(_j), _y3 + 1.5, _x27 + (_len5 - gap) * ska(_j), _y3 - 3 + _height);
						}
						// Skill name
						_len5 = save * ska(_max);
						var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
						text.skillNames[ticker - 1].draw(name, Math.round(_x27 + _len5 / 2) + 0.5, _y3 + _height / 2, _height - 5, textcolor, 'center', true);
						// Skill key
						text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x27 + _len5 - _height * 0.25) - 1.5, _y3 + _height / 2, _height - 5, textcolor, 'right', true);
						if (textcolor === color.guiwhite) {
							// If it's active
							global.clickables.stat.place(ticker - 1, _x27, _y3, _len5, _height);
						}
						// Skill value
						if (level) {
							text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x27 + _len5 + 4) + 0.5, _y3 + _height / 2, _height - 5, col, 'left', true);
						}
						// Move on 
						_y3 -= _height + _vspacing;
					}
				});
				global.clickables.hover.place(0, 0, _y3, 0.8 * _len5, 0.8 * (global.screenHeight - _y3));
				if (_gui.points !== 0) {
					// Draw skillpoints to spend
					text.skillPoints.draw('x' + _gui.points, Math.round(_x27 + _len5 - 2) + 0.5, Math.round(_y3 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
				}
			}

			{
				...
				var _y4 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				// Handle exp
				// Draw the exp bar
				drawBar(_x28, _x28 + _len6, _y4 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x28, _x28 + _len6, _y4 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x28, _x28 + _len6 * _gui.__s.getProgress(), _y4 + _height2 / 2, _height2 - 3.5, color.gold);
				// Draw the class type
				text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x28 + _len6 / 2, _y4 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', true);
				_height2 = 14;
				_y4 -= _height2 + _vspacing2;
				// Draw the %-of-leader bar
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * 0.9, _y4 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * 0.9, _y4 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * (0.1 + 0.8 * (max ? Math.min(1, _gui.__s.getScore() / max) : 1)), _y4 + _height2 / 2, _height2 - 3.5, color.green);
				// Draw the score
				text.score.draw('Score: ' + util.handleLargeNumber(_gui.__s.getScore()), _x28 + _len6 / 2, _y4 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', true);
				// Draw the name
				ctx.lineWidth = 4;
				text.name.draw(player.name, Math.round(_x28 + _len6 / 2) + 0.5, Math.round(_y4 - 10 - _vspacing2) + 0.5, 32, color.guiwhite, 'center');
			}

			{
				...
				var _y5 = global.screenHeight - _height3 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x29 + j++ * _len7 / _W, _y5 + _i9 * _height3 / _H, _len7 / _W, _height3 / _H);
					});
					_i9++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x29, _y5, _len7, _height3);
				minimap.forEach(function (o) {
					if (o[2] === 17) {
						ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.globalAlpha = 0.8;
						drawGuiRect(_x29 + o[0] / global.gameWidth * _len7, _y5 + o[1] / global.gameHeight * _height3, 1, 1);
					} else {
						ctx.strokeStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.lineWidth = 1;
						ctx.globalAlpha = 1;
						drawGuiRect(_x29 + o[0] / global.gameWidth * _len7 - 1, _y5 + o[1] / global.gameWidth * _height3 - 1, 3, 3, true);
						ctx.lineWidth = 3;
					}
				});
				ctx.globalAlpha = 1;
				...
				_x29 + player.x / global.gameWidth * _len7 - 1, _y5 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				ctx.fillStyle = color.black;
				drawGuiRect(_x29, _y5, _len7, _height3, true); // Border
...

			{
				...
				var _y6 = spacing + _height4 + 7;
				text.lbtitle.draw('Leaderboard:', Math.round(_x30 + _len8 / 2) + 0.5, Math.round(_y6 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i10 = 0;
				lb.data.forEach(function (entry) {
					drawBar(_x30, _x30 + _len8, _y6 + _height4 / 2, _height4 - 3 + config.graphical.barChunk, color.black);
					drawBar(_x30, _x30 + _len8, _y6 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x30, _x30 + _len8 * shift, _y6 + _height4 / 2, _height4 - 3.5, entry.barcolor);
					// Leadboard name + score 
					text.leaderboard[_i10++].draw(entry.label + ': ' + util.handleLargeNumber(Math.round(entry.score)), _x30 + _len8 / 2, _y6 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', true);
					// Mini-image
					var scale = _height4 / entry.position.axis,
					    xx = _x30 - 1.5 * _height4 - scale * entry.position.middle.x * 0.707,
					    yy = _y6 + 0.5 * _height4 + scale * entry.position.middle.x * 0.707;
					drawEntity(xx, yy, entry.image, 1 / scale, scale * scale / entry.image.size, -Math.PI / 4, true);
					// Move down
					_y6 += _vspacing3 + _height4;
				});
			}

			{
				...
				if (_gui.upgrades.length > 0) {
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y7 > yo) yo = _y7;
						xxx = _x31;
						global.clickables.upgrade.place(_i11++, _x31, _y7, _len9, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x31, _y7, _len9, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x31, _y7, _len9, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x31, _y7 + _height5 * 0.6, _len9, _height5 * 0.4);
						ctx.globalAlpha = 1;
						...
						var picture = getEntityImageFromMockup(model, _gui.color),
						    ...
						    yy = _y7 + 0.5 * _height5 - scale * position.middle.x * Math.sin(upgradeSpin);
						drawEntity(xx, yy, picture, 1, scale / picture.size, upgradeSpin, true);
						// Tank name
						text.upgradeNames[_i11 - 1].draw(picture.name, _x31 + 0.9 * _len9 / 2, _y7 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						// Upgrade key
						text.upgradeKeys[_i11 - 1].draw('[' + getClassUpgradeKey(_ticker) + ']', _x31 + _len9 - 4, _y7 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'right');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x31, _y7, _len9, _height5, true); // Border
						if (_ticker++ % 2) {
							_y7 -= _height5 + internalSpacing;
